...
const Cart = ({cartItems,cart,total,removeItem,removeId}) => {
    return (
        cartItems?
        <div className='w-full min-h-[300px] h-auto bg-white flex py-[20px] flex-col items-center'>
            {cart.map((item,index)=><Card3 key={index} removeItem={removeItem} cartItems={cartItems} removeIndex={index} item={item}/>)}
